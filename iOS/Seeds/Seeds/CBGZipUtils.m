...
+(NSData*) gzipData:(NSData *)pUncompressedData
{
    ...
    {
		DDLogWarn(@"%s: Error: Can't compress an empty or nil NSData object", __func__);
		return nil;
	}
	...
	if (initError != Z_OK)
    {
		...
		DDLogWarn(@"%s:deflateInit2() Error: \"%@\" Message: \"%s\"",__func__,errorMsg,zlibStreamStruct.msg);
		return nil;
	}
	...
	if (deflateStatus != Z_STREAM_END) 
	{
        ...
        DDLogWarn(@"%s:zlib error while attempting compression: \"%@\" Message: \"%s\"", __func__, errorMsg, zlibStreamStruct.msg);
