...
public class JsonResponseSeedsToCart extends JsonResponseBase
{
    ...
    public void responseNormalRequest()
    {
        ...
        {
            LogUtil.severe("seedList is null or empty");
            return;
        }
        ...
        {
            if (result == SeedToCartResult.invalid)
            {
                responseError(ErrorCode.IllegalMessageBody, "Invalid seedId.");
            }
            else if (result == SeedToCartResult.failed)
            {
    ...
    private SeedToCartResult saveRssRequest(String seedId, String cartId)
    {
        ...
        sql = "Insert into " + TableName.Cart +"("+ TableColumnName.cartId + ", "+ TableColumnName.seedId +") " + "values ('" + cartId + "', " + seedId + ")";
